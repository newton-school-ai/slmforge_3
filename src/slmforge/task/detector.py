...
def analyze_schema_and_content(sample_data: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    if any(k in key_lower for k in ["label", "target", "class", "category"]):
        ...
        label_key = next((k for k, kl in zip(keys, key_lower) if kl in ["label", "target", "class", "category"]), None)
        if label_key:
            unique_labels = set(str(row.get(label_key)) for row in sample_data if row.get(label_key) is not None)
    ...
    if any(k in key_lower for k in ["summary", "abstract"]) and any(k in key_lower for k in ["text", "document", "article"]):
        scores["summarisation"] += 0.7
        
        summary_key = next((k for k, kl in zip(keys, key_lower) if kl in ["summary", "abstract"]), None)
        text_key = next((k for k, kl in zip(keys, key_lower) if kl in ["text", "document", "article"]), None)
    ...
    if "messages" in key_lower or "conversations" in key_lower:
        ...
        msg_key = next((k for k, kl in zip(keys, key_lower) if kl in ["messages", "conversations"]), None)
        if msg_key:
            sample_val = sample_data[0].get(msg_key)
            if isinstance(sample_val, list) and len(sample_val) > 0:
                if isinstance(sample_val[0], dict) and "role" in sample_val[0] and "content" in sample_val[0]:
                    scores["chat"] += 0.2
